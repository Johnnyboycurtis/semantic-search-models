...
retrieval_evaluator = NanoBEIREvaluator(
    dataset_names=["MSMARCO", "HotpotQA"], batch_size=32
)
...
nli_train_dataset = load_dataset(
    "sentence-transformers/all-nli", "pair-score", split="train"
)
nli_eval_dataset = load_dataset(
    "sentence-transformers/all-nli", "pair-score", split="dev"
)
...
nli_eval_dataset = nli_eval_dataset.map(
    combine_sentences, batched=True, remove_columns=nli_eval_dataset.column_names
)
...
wikipedia_train_dataset = load_dataset(
    "sentence-transformers/wikipedia-en-sentences", split="train"
)
...
philosophy_ds = load_dataset(
    "johnnyboycurtis/Philosophical-STS-Text-Pairs", split="train"
)
...
if (
    ...
    pca_sentences = (
        nli_train_dataset[:20000]["sentence"]
        + wikipedia_train_dataset[:20000]["sentence"]
    )
    ...
    logging.info(
        f"Teacher Performance with {teacher_model.get_sentence_embedding_dimension()} dimensions:"
    )
    dev_evaluator_stsb(teacher_model)
...
# 5. & 6. Load Pre-computed Dataset
from datasets import load_from_disk

dataset_path = "datasets/distillation_train_dataset"

if Path(dataset_path).exists():
    logging.info(f"Loading pre-computed dataset from {dataset_path}...")
    train_dataset = load_from_disk(dataset_path)
    print(train_dataset.column_names)
    # train_dataset.set_format("torch", columns=["label"], output_all_columns=True)

else:
    logging.error(f"Dataset not found at {dataset_path}. Please check the path.")
    # Fallback logic or exit
    exit()
    train_dataset = train_dataset.shuffle(123).select(range(4000000))
...
dev_evaluator_mse = evaluation.MSEEvaluator(
    eval_sentences, eval_sentences, teacher_model=teacher_model
)
dev_evaluator = evaluation.SequentialEvaluator(
    [dev_evaluator_stsb, dev_evaluator_mse, retrieval_evaluator]
)
