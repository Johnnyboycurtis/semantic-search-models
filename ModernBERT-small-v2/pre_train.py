...
if not Path(stage_1_model_path).exists():
    logging.warning(
        f"Model path not found at {stage_1_model_path}. Ensure this is correct."
    )
...
gooaq_dataset = gooaq_dataset.rename_columns(
    {"question": "anchor", "answer": "positive"}
)
...
logging.info(
    f"Training with {len(train_dataset)} datasets: {list(train_dataset.keys())}"
)
...
inner_loss_symmetric = losses.CachedMultipleNegativesSymmetricRankingLoss(
    model, mini_batch_size=64
)
loss_symmetric = losses.MatryoshkaLoss(
    model, inner_loss_symmetric, matryoshka_dims=mrl_dims
)
